...
LM_tem <- function(S, yv, k, modBasic, tol, maxit, piv, Pi, Phi, algorithm, profile_pars) {
  ...
  while (abs((lk - lko)/max(lk, lko)) > tol & it < maxit) {
    ...
    step_i <- LM_tem_step(Sv = Sv, yv = yv, n = n, ns = ns, r = r, TT = TT, k = k,
                          bv = bv, b = b, piv = piv, Pi = Pi, Phi = Phi, Psi = Psi, pv = pv, L = L, mod = modBasic, temp = temp)
